...
> **Dernière version : `1.7.348`** — 10 septembre 2026
...
| Champ | Valeur |
|---|---|
| Version app (affichée) | `1.7.348` |
| Cache Service Worker | `superprint-shell-v1.7.348-no-whatsapp` |
| Query JS (`main.js`) | `?v=20260910-v348-delete-fix` |
| Badge preview (`spVersionBadge`) | `v1.7.348` |
| Splash screen | `v1.7.348` |
| Préférences (panneau SUPERPRINT) | `v1.7.348` |
| Préférences (bas de page) | `SuperPrint v1.7.348 — 10 septembre 2026` |
| Onboarding (`V 1.7.348`) | `1.7.348` |
| Landing (`softwareVersion` + footer) | `1.7.348` |
| Documentation | `v1.7.348` |
| `version.txt` | `1.7.348` |
| `llms.txt` / `llms-full.txt` | `1.7.348 (September 2026)` |
| `package.json` (sp213-local) | `1.7.348` |
...
Ex. si le badge affichait `1.7.271`, bump vers `1.7.348` minimum.
...
1. Choisir le nouveau numéro (ex. `1.7.348`) — **toujours > au cache SW actuel**.
2. Mettre à jour TOUS les fichiers de la section 2 (web + copie locale).
3. Vérifier : `grep -r "1.7.348" superprint/` → plus aucune occurrence (sauf changelog main.js).
