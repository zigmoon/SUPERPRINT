...
> **Dernière version : `1.7.488`** — 19 septembre 2026
...
| Champ | Valeur |
|---|---|
| Version app (affichée) | `1.7.488` |
| Cache Service Worker (app **et** racine) | `superprint-shell-v1.7.488-faq-a-part` |
| Query JS (`main.js`) **et** CSS (`main.css`) | `?v=20260919-v488-faq-a-part` |
| Badge preview (`spVersionBadge`) | `v1.7.488` — 5 marqueurs : commentaire, `title` (`JS v456`), `data-sp-js="v456"`, `data-sp-sw`, texte |
| Splash screen | `1.7.488` |
| Onboarding | `LAYOUT EDITOR — V 1.7.488` + `V 1.7.488 \| 09 2026` |
| JSON-LD `softwareVersion` (lanceur `index.html` + `landing.html` + `app/index.html` + `app/landing.html`) | `1.7.488` |
| Documentation (racine **et** `app/`) | `v1.7.488` (pastille + pieds de page datés + 214 numéros de ligne) |
| `api.html` (pastille `.version`) | `v1.7.488` |
| `version.txt` | `1.7.488` |
| `llms.txt` / `llms-full.txt` (racine **et** `app/`) | `1.7.488 (September 2026)` |
| `package.json` + `package-lock.json` (sp213-local) | `1.7.488` |
...
- ⚠️ **PLUS de `sp213-local/studio.html`** (retiré en 1.7.488) : il faisait doublon avec le
