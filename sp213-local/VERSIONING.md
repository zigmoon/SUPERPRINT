...
> **Dernière version : `1.7.556`** — 19 septembre 2026
...
| Champ | Valeur |
|---|---|
| Version app (affichée) | `1.7.556` |
| Cache Service Worker (app **et** racine) | `superprint-shell-v1.7.556-recadrage-conserve` |
| Query JS (`main.js`) **et** CSS (`main.css`) | `?v=20261026-v556-recadrage-conserve` |
| Badge preview (`spVersionBadge`) | `v1.7.556` — 5 marqueurs : commentaire, `title` (`JS v456`), `data-sp-js="v456"`, `data-sp-sw`, texte |
| Splash screen | `1.7.556` |
| Onboarding | `LAYOUT EDITOR — V 1.7.556` + `V 1.7.556 \| 09 2026` |
| JSON-LD `softwareVersion` (lanceur `index.html` + `landing.html` + `app/index.html` + `app/landing.html`) | `1.7.556` |
| Documentation (racine **et** `app/`) | `v1.7.556` (pastille + pieds de page datés + 214 numéros de ligne) |
| `api.html` (pastille `.version`) | `v1.7.556` |
| `version.txt` | `1.7.556` |
| `llms.txt` / `llms-full.txt` (racine **et** `app/`) | `1.7.556 (September 2026)` |
| `package.json` + `package-lock.json` (sp213-local) | `1.7.556` |
...
- ⚠️ **PLUS de `sp213-local/studio.html`** (retiré en 1.7.556) : il faisait doublon avec le
