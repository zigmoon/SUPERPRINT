...
> **Dernière version : `1.7.347`** — 10 septembre 2026
...
| Champ | Valeur |
|---|---|
| Version app (affichée) | `1.7.347` |
| Cache Service Worker | `superprint-shell-v1.7.347-no-whatsapp` |
| Query JS (`main.js`) | `?v=20260910-v347-delete-fix` |
| Badge preview (`spVersionBadge`) | `v1.7.347` |
| Splash screen | `v1.7.347` |
| Préférences (panneau SUPERPRINT) | `v1.7.347` |
| Préférences (bas de page) | `SuperPrint v1.7.347 — 10 septembre 2026` |
| Onboarding (`V 1.7.347`) | `1.7.347` |
| Landing (`softwareVersion` + footer) | `1.7.347` |
| Documentation | `v1.7.347` |
| `version.txt` | `1.7.347` |
| `llms.txt` / `llms-full.txt` | `1.7.347 (September 2026)` |
| `package.json` (sp213-local) | `1.7.347` |
...
Ex. si le badge affichait `1.7.271`, bump vers `1.7.347` minimum.
...
1. Choisir le nouveau numéro (ex. `1.7.347`) — **toujours > au cache SW actuel**.
2. Mettre à jour TOUS les fichiers de la section 2 (web + copie locale).
3. Vérifier : `grep -r "1.7.347" superprint/` → plus aucune occurrence (sauf changelog main.js).
