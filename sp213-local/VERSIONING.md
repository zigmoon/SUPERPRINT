...
> **Dernière version : `1.7.346`** — 10 septembre 2026
...
| Champ | Valeur |
|---|---|
| Version app (affichée) | `1.7.346` |
| Cache Service Worker | `superprint-shell-v1.7.346-no-whatsapp` |
| Query JS (`main.js`) | `?v=20260910-v346-delete-fix` |
| Badge preview (`spVersionBadge`) | `v1.7.346` |
| Splash screen | `v1.7.346` |
| Préférences (panneau SUPERPRINT) | `v1.7.346` |
| Préférences (bas de page) | `SuperPrint v1.7.346 — 10 septembre 2026` |
| Onboarding (`V 1.7.346`) | `1.7.346` |
| Landing (`softwareVersion` + footer) | `1.7.346` |
| Documentation | `v1.7.346` |
| `version.txt` | `1.7.346` |
| `llms.txt` / `llms-full.txt` | `1.7.346 (September 2026)` |
| `package.json` (sp213-local) | `1.7.346` |
...
Ex. si le badge affichait `1.7.271`, bump vers `1.7.346` minimum.
...
1. Choisir le nouveau numéro (ex. `1.7.346`) — **toujours > au cache SW actuel**.
2. Mettre à jour TOUS les fichiers de la section 2 (web + copie locale).
3. Vérifier : `grep -r "1.7.346" superprint/` → plus aucune occurrence (sauf changelog main.js).
