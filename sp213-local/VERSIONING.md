...
> **Dernière version : `1.7.497`** — 19 septembre 2026
...
| Champ | Valeur |
|---|---|
| Version app (affichée) | `1.7.497` |
| Cache Service Worker (app **et** racine) | `superprint-shell-v1.7.497-bureau-et-pied-de-page` |
| Query JS (`main.js`) **et** CSS (`main.css`) | `?v=20260920-v497-bureau-et-pied-de-page` |
| Badge preview (`spVersionBadge`) | `v1.7.497` — 5 marqueurs : commentaire, `title` (`JS v456`), `data-sp-js="v456"`, `data-sp-sw`, texte |
| Splash screen | `1.7.497` |
| Onboarding | `LAYOUT EDITOR — V 1.7.497` + `V 1.7.497 \| 09 2026` |
| JSON-LD `softwareVersion` (lanceur `index.html` + `landing.html` + `app/index.html` + `app/landing.html`) | `1.7.497` |
| Documentation (racine **et** `app/`) | `v1.7.497` (pastille + pieds de page datés + 214 numéros de ligne) |
| `api.html` (pastille `.version`) | `v1.7.497` |
| `version.txt` | `1.7.497` |
| `llms.txt` / `llms-full.txt` (racine **et** `app/`) | `1.7.497 (September 2026)` |
| `package.json` + `package-lock.json` (sp213-local) | `1.7.497` |
...
- ⚠️ **PLUS de `sp213-local/studio.html`** (retiré en 1.7.497) : il faisait doublon avec le
