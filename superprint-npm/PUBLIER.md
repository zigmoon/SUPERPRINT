...
C'est tout. Le package `superprint` est en ligne (version actuelle : **1.0.107**).
...
| `version.txt` | Version en ligne (`1.7.556`) — permet la détection de mise à jour. À déployer en même temps que le zip. |
...
> ⚠️ **Ordre impératif** : téléverser le zip ET le site **avant** d'avancer `MIN_APP_VERSION` dans `cli.mjs`, sinon `npx superprint` s'arrête sur « downloaded version is invalid or older than the online version ». Plancher actuel : **1.7.547**, aligné sur ce qui est EN LIGNE (zip **1.7.547**, 61 667 847 octets, et `version.txt` = 1.7.547). La nouvelle archive est prête : zip **1.7.556**, 61 680 392 octets, vérifié le 26/09/2026 (`superprint/sp213-local.zip`) — dès qu'elle est sur app.zigmoon.com et que `version.txt` affiche 1.7.556 sur superprint.cc, passer `MIN_APP_VERSION` à `'1.7.556'` (une ligne dans `cli.mjs`).
