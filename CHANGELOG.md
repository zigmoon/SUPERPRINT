...
_Copy-paste and templates: serialisation stored selection-relative coordinates instead of page coordinates_

### Fixed
- **A multiple selection corrupted the document on every save** (user report: "dans les templates, lorsque je souhaite remplacer une image, les pages se déconstruisent avec des blocs qui changent de place en x et y mais aussi en avant et arrière plan"). Audit finding, **measured**: while a `fabric.ActiveSelection` is active, Fabric.js gives each child **relative** coordinates centred on the selection — a rectangle placed at (200, 300) reports `left: -103.3, top: -84.12`, and `getCenterPoint()`, `getPointByOrigin()` and `getBoundingRect()` all return those relative values. `toObject()` wrote them as-is, so **any** save taken during a multiple selection (`saveState`, autosave, `Ctrl+S`, page change, paste, template action, image replacement) stored `left: -92, top: -68` for an object that was really at (200, 300). On the next render the blocks landed off-page, were clamped back by force and ended up looking restacked — exactly the "pages fall apart" the user saw. Fix: `spCoordsMondeSerialisation()` is now called by `serializeObject` **and** by `saveSpreadContent`, so every serialisation writes world coordinates. It is exact, not approximate: `calcTransformMatrix()` carries the object's world centre in `(m[4], m[5])` and `translateToOriginPoint()` brings it back to the origin corner (verified on a 12°-rotated object). Measured after the fix: with the selection still active, `saveAllPages()` writes (200, 300) / (210, 410) and `renderAllPages()` moves nothing.
- **`saveSpreadContent` split objects across pages using the relative left edge.** The page-assignment maths (`objLeftEdge` / `objRightEdge` / `objCenterX`) read `obj.left`, which is selection-relative while a selection is active, so an object could be assigned to a neighbouring page during a spread save. It now uses the world left edge. Measured: a 2-column block created on page 1 at (88, 88) is pasted on page 2 at (98, 98) and stays on page 2 through a full page round trip.
- **Copy-paste left the source objects off-page** (user report: "lorsque je copie colle un bloc texte d'une page vers une autre page avec du texte dépassant, le bloc texte se copie mal avec parfois un texte hors du bloc et parfois pas de texte du tout"). Audit finding, **measured**: `_spSnapshotSourceXforms()` was taken **before** the paste's `discardActiveObject()`, so it captured selection-relative coordinates, and `_spRestoreSourceXforms()` re-applied them at the end of the paste. The source objects were left at relative coordinates with `group: null` — off-page, with no way back. That is the "sometimes" in the report: a copy of a **single** object is never re-anchored, so only multi-selection copies were hit. Fix: `_spSnapshotXform()` stores world coordinates. Measured: sources stay at (150, 150) / (150, 350), the copy lands at (160, 160) / (160, 360), and `pages[0]` holds world coordinates only.
- **A pasted overflowing text frame carried no overflow marker** (user report: "l'indicateur montrant qu'il y a un texte dépassant doit toujours être collé au bloc texte"). Audit finding, **measured**: both multi-paste paths (`__spMultiClipboard` and `isMulti`) finished without calling `updateOverflowIndicator`, so the pasted frame came back with `repere: false` — the user had no warning that the text overflowed. Fix: both paths now run the indicator for every pasted `textbox` / `text` / `i-text`. Measured: `repere: true`.
- **The overflow marker was positioned from the bounding rectangle, which is selection-relative too.** Measured error: about 460 px up and 260 px left while the frame sat inside a multiple selection, so the marker floated far away from the frame. Fix: new global helper `spRectMondeObjet()` returns the frame's **world** rectangle (it shifts `getBoundingRect()` by the world-minus-relative delta), and both positioning sites use it — the initial placement and the `__overflowListener` that follows every edit. Measured: the marker sits at (361, 221) = the frame's exact bottom-right corner, while the frame is inside a multiple selection.
- **Stacking order was investigated and cleared.** The user also saw blocks change "en avant et arrière plan". Measured before / during / after a multiple selection: the canvas stack order stays `#111111, #222222, #333333, #444444` — identical, because Fabric does not re-add selection children to the canvas. The apparent restacking came from the wrong positions above (blocks landing on a neighbouring page or off-page), not from a sort. No z-order code was changed.

_These three reports shared a single root cause; two independent serialisation sites and one snapshot site were repaired._
